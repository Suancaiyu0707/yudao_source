...
* 在 [TODO 【3008】]() 详细解析 GatewayProperties 的代码实现。
...
* 每个 `@Bean` 注解后的数字为 Bean 的**初始化顺序**。
* 第 1 至 5 行 ：使用 GatewayProperties Bean ，创建一个类型为 `org.springframework.cloud.gateway.config.PropertiesRouteDefinitionLocator`  的 Bean 对象。在 [TODO 【3008】]() 详细解析 PropertiesRouteDefinitionLocator 的代码实现。
