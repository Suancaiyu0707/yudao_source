title: 精尽 Dubbo 源码分析 —— 注册中心（一）之抽象 API
date: 2018-08-01
tags:
categories: Dubbo
...
* `#subscribe(url, NotifyListener)` 方法，**订阅**符合条件的已注册数据，当有注册数据变更时自动推送。
    ...
    * 在 `URL.parameters.category` 属性上，表示订阅的数据分类。目前有四种类型：
        * `consumers` ，服务消费者列表。
        * `providers` ，服务提供者列表。
        * `routers` ，[路由规则](http://dubbo.io/books/dubbo-user-book/demos/routing-rule.html)列表。 
        * `configurations` ，[配置规则](http://dubbo.io/books/dubbo-user-book/demos/config-rule.html)列表。
...
* `properties` 属性，*见代码注释*。
    ...
    * 数据键值
        * 大多数情况下，键为服务消费者的 URL 的服务键( `URL#serviceKey()` )，对应的值为服务提供者列表、[路由规则](http://dubbo.io/books/dubbo-user-book/demos/config-rule.html)列表、[配置规则](http://dubbo.io/books/dubbo-user-book/demos/routing-rule.html)列表。
...
该信息用于 [Dubbo QOS](http://dubbo.io/books/dubbo-user-book/references/qos.html) 使用，例如将 JVM 进程中，**自己**的服务提供者下线，又或者查询自己的服务提供者和消费者列表。

* [《Dubbo 用户指南 —— 在线运维命令 - QOS》](http://dubbo.io/books/dubbo-user-book/references/qos.html)
* 后续会有文章分享 QOS ，本文不多啰嗦。
