...
- [666. 拓展](http://www.iocoder.cn/Dubbo/spi/)
...
```Java
 ...
 62:             // 将配置的自定义在自动激活的拓展对象们前面。例如，<dubbo:service filter="demo,default,demo2" /> ，则 DemoFilter 就会放在默认的过滤器前面。ps：这里只是举个例子，实际配置 `default` 配置在 filter 属性中会报错。
