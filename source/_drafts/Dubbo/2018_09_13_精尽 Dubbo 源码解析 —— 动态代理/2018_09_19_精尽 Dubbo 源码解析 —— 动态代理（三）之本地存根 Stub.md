title: 精尽 Dubbo 源码分析 —— 动态代理（三）之本地存根 Stub
date: 2018-09-19
tags:
categories: Dubbo
permalink: Dubbo/proxy-local-stub

-------

摘要: 原创出处 http://www.iocoder.cn/Dubbo/proxy-local-stub/ 「芋道源码」欢迎转载，保留摘要，谢谢！

- [1. 概述](http://www.iocoder.cn/Dubbo/proxy-local-stub/)
- [2. StubProxyFactoryWrapper](http://www.iocoder.cn/Dubbo/proxy-local-stub/)
  - [2.1 构造方法](http://www.iocoder.cn/Dubbo/proxy-local-stub/)
  - [2.2 getProxy](http://www.iocoder.cn/Dubbo/proxy-local-stub/)
  - [2.3 getInvoker](http://www.iocoder.cn/Dubbo/proxy-local-stub/)
- [3. 校验 Stub 配置](http://www.iocoder.cn/Dubbo/proxy-local-stub/)
- [666. 彩蛋](http://www.iocoder.cn/Dubbo/proxy-local-stub/)
...
本文接 [《精尽 Dubbo 源码分析 —— 动态代理（二）之 JDK》](http://www.iocoder.cn/Dubbo/proxy-jdk/?self) 一文，分享使用 Dubbo **本地存根( Stub )**的特性。😁 当然，从标题我们就可以看出，实现的原理是基于**动态代理**的机制。

在 [《Dubbo 用户指南 —— 本地存根》](https://dubbo.gitbooks.io/dubbo-user-book/demos/local-stub.html) 中，已经非常详尽的分享了**本地存根**的概念和使用，本文就不重复介绍啦。😈 文档有一点点小小的错误，在 Spring 配置文件的配置方式应该是如下：
...
[`com.alibaba.dubbo.rpc.proxy.wrapper.StubProxyFactoryWrapper`](TODO ) ，实现 ProxyFactory 接口，存根代理工厂**包装器**实现类。
...
* 【**第一步**】第 5 行：调用 `proxyFactory#getProxy(invoker)` 方法，获得 Service Proxy 对象。这个过程，就是我们在 [《精尽 Dubbo 源码解析 —— 动态代理》](#) 前两篇看到的内容。
