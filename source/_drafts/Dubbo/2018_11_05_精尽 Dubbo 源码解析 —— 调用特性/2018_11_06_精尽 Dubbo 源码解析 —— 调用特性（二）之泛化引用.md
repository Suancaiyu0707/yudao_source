title: 精尽 Dubbo 源码分析 —— 调用特性（二）之泛化引用
date: 2018-11-06
tags:
categories: Dubbo
...
在 [`dubbo-generic-reference-demo-provider`](TODO) ，我们提供了例子。普通的服务提供者，不需要做任何处理，胖友自己查看。

**服务消费者**

在 [`dubbo-generic-reference-demo-consumer`](TODO) ，我们提供了例子。我们挑**重点**的地方说。
...
[`com.alibaba.dubbo.rpc.filter.GenericImplFilter`](todo) ，实现 Filter 接口，服务消费者的泛化调用过滤器。代码如下：
...
[`com.alibaba.dubbo.rpc.filter.GenericFilter`](TODO) ，实现 Filter 接口，服务消费者的泛化调用过滤器。代码如下：
