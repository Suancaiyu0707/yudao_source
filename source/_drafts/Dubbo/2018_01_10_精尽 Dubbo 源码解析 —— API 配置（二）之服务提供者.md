...
* 具体属性的解释，参见 [《Dubbo 用户指南 —— dubbo:protocol》](http://dubbo.io/books/dubbo-user-book/references/xml/dubbo-protocol.html) 文档。
...
* **部分**属性的解释，参见 [《Dubbo 用户指南 —— dubbo:method》](http://dubbo.io/books/dubbo-user-book/references/xml/dubbo-method.html) 文档。
...
* 具体属性的解释，[《Dubbo 用户指南 —— dubbo:method》](http://dubbo.io/books/dubbo-user-book/references/xml/dubbo-method.html) 文档。
...
* 具体属性的解释，**需要寻找**在 [《Dubbo 用户指南 —— dubbo:service》](http://dubbo.io/books/dubbo-user-book/references/xml/dubbo-service.html) 或 [《Dubbo 用户指南 —— dubbo:reference》](http://dubbo.io/books/dubbo-user-book/references/xml/dubbo-reference.html) 文档。
...
* 具体属性的解释，**需要寻找**在 [《Dubbo 用户指南 —— dubbo:service》](http://dubbo.io/books/dubbo-user-book/references/xml/dubbo-service.html) 或 [《Dubbo 用户指南 —— dubbo:provider》](https://dubbo.gitbooks.io/dubbo-user-book/references/xml/dubbo-provider.html) 文档。
...
* 具体属性的解释，参见 [《Dubbo 用户指南 —— dubbo:provider》](http://dubbo.io/books/dubbo-user-book/references/xml/dubbo-provider.html) 文档。
...
* 具体属性的解释，参见 [《Dubbo 用户指南 —— dubbo:service》](http://dubbo.io/books/dubbo-user-book/references/xml/dubbo-service.html) 文档。
...
* 第 17 至 22 行：当配置延迟暴露( `delay > 0` )时，使用 [`delayExportExecutor`](https://github.com/YunaiV/dubbo/blob/9d38b6f9f95798755141d6140e311e8fd51fecc1/dubbo-config/dubbo-config-api/src/main/java/com/alibaba/dubbo/config/ServiceConfig.java#L86) **延迟**调度，调用 `#doExport()` 方法。
    * [《Dubbo 用户指南 —— 延迟暴露》](http://dubbo.io/books/dubbo-user-book/demos/delay-publish.html) 
...
```Java
 ...
 93:         }
 94: 
 95:         String[] methods = Wrapper.getWrapper(interfaceClass).getMethodNames(); // 【TODO 8003】Wrapper
...
* 第 103 至 110 行：将 `token` 添加到 `map` 中。
    * [《Dubbo 用户指南 —— 令牌验证》](http://dubbo.io/books/dubbo-user-book/demos/token-authorization.html)
* 第 111 至 115 行：当协议为 `injvm` 时，添加 `notify = false` 到 `map` 中，表示不注册，不通知。
    * [《Dubbo 用户指南 —— 本地调用》](http://dubbo.io/books/dubbo-user-book/demos/local-call.html) 
...
* 第 3 至 7 行：从 `MAINFEST.MF` 中获得版本号。以 [spring-boot-starter-1.5.1.0.RELEASE.jar](http://central.maven.org/maven2/org/springframework/boot/spring-boot-starter/1.5.10.RELEASE/spring-boot-starter-1.5.10.RELEASE.jar) 举例子：[MAINFEST.MF](http://www.iocoder.cn/images/Dubbo/2018_01_10/01.png)
* 第 8 至 36 行：若获取不到，从 jar 包**命名**中**可能**带的版本号作为结果。例如上面的例子，`1.5.1.0.RELEASE` 。
