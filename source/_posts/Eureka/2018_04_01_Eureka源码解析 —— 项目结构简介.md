...
* `com.netflix.discovery` 包：Eureka-Client 的注册与发现相关功能。
    ...
    * `com.netflix.discovery.guice` 包：Eureka 使用 [Google Guice](https://github.com/google/guice) 实现依赖注入。一方面 Guice 是轻量级的依赖注入框架，另一方面避免和业务代码的 Spring 版本冲突。
