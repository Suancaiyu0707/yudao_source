...
> [](../../../images/Eureka/2018_06_01/01.jpeg)
...
* `scheduler`，定时任务服务，用于定时触发心跳( 续租 )。细心如你，会发现任务提交的方式是 `ScheduledExecutorService#schedule(...)` 方法，**无法固定频率执行心跳**！！！答案在 [「2.3 TimedSupervisorTask」](#) 揭晓。
...
* 调用 `#renew` 方法，执行续租逻辑。实现代码如下：
    ...
    * 调用 `AbstractJerseyEurekaHttpClient#sendHeartBeat(...)` 方法，发起**续租请求**，实现代码如下：
        ...
        ```
        * PUT 请求 Eureka-Server 的 `apps/${APP_NAME}/${INSTANCE_INFO_ID}` 接口，参数为 `status`、`lastDirtyTimestamp`，实现续租。
...
[](../../../images/Eureka/2018_06_01/02.png)
